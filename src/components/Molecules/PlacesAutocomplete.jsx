...
const fetch = debounce((request, callback) => {
  // Use the legacy AutocompleteService instead of the new API
  ...
  autocompleteService.getPlacePredictions(request, (results, status) => {
    if (status === window.google.maps.places.PlacesServiceStatus.OK && results) {
      ...
      // Fallback to fake data if quota is exceeded
...
export default function PlacesAutocomplete({ onSelect, selectedMarker }) {
  ...
  React.useEffect(() => {
    if (selectedMarker && selectedMarker.title) {
      setInputValue(selectedMarker.title);
      setValue({ description: selectedMarker.title });
    } else if (!selectedMarker?.title) {
      setInputValue("");
      setValue(null);
    }
  }, [selectedMarker]);
  ...
  return (
    <Autocomplete
      ...
      onChange={(event, newValue) => {
        console.log("Selected suggestion:", newValue);
        setOptions(newValue ? [newValue, ...options] : options);
        setValue(newValue);

        if (newValue && newValue.place_id && window.google) {
      ...
      renderOption={(props, option) => {
        ...
        const parts =
          mainText && matches.length
            ? parse(
                mainText,
                matches.map((m) => [m.offset, m.offset + m.length])
              )
            : [{ text: mainText, highlight: false }];
